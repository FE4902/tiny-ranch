...
- pnpm command: `pnpm run report:retention:health`
...
Running `pnpm run report:retention:health` writes:
...
In CI, use `pnpm run report:retention:health -- --run-playwright` so the snapshot also includes:
...
| `passedTests`, `failedTests` (save migration) | Save migration decode/re-save matrix (`tests/smoke/save-migration-matrix.spec.ts`) | `pnpm exec playwright test --project=desktop-chromium tests/smoke/save-migration-matrix.spec.ts --reporter=json` |
| `passedTests`, `failedTests` (memory gate) | Mobile memory/frame drift gate (`tests/smoke/retention-memory-gate.spec.ts`) | `pnpm exec playwright test --project=mobile-chromium tests/smoke/retention-memory-gate.spec.ts --reporter=json` |
...
3. Re-run:
   ...
   - `pnpm run report:retention:health -- --run-playwright` (when Playwright dependencies are available)
...
- command: `pnpm run gate:retention:baseline-diff`
